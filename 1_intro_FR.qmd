...
# get total populations across all age and sex
total_pops <- d  |>  
  filter(region=="Japan"|region=="Italy") |> 
  pivot_longer(x0:x100, names_to = "age", values_to = "pop") |> 
  mutate(age = as.numeric(str_remove(age, "x"))) |> 
  group_by(region, year) |> 
  summarize(total_pop = sum(pop))
...
d_long|> 
  filter(region=="Japan"|region=="Italy", year == 2019)  |>  
  mutate(population=ifelse(sex=="Male", -pop, pop)) |> 
ggplot(aes(x = age, y = population, fill = sex)) + 
  facet_wrap(~region)+
  geom_bar(stat="identity")+
  ...
  scale_y_continuous(breaks = seq(-4000, 4000, 1000), 
                     labels = c(seq(4000, 0, -1000), seq(1000, 4000, 1000))) + 
  scale_fill_brewer(palette = "Set1") 
ggsave(here("plots", "JAP_ITA_pyramid.pdf"))
```
```{r}
d_long |> 
  filter(region=="Japan"|region=="Italy", year%in% c(1960, 1990, 2015, 2020))  |>  
  mutate(population=ifelse(sex=="Male", -pop, pop)) %>% 
ggplot(aes(x = age, y = population, fill = sex)) + 
  facet_grid(year~region)+
  geom_bar(stat="identity")+
  ...
  scale_y_continuous(breaks = seq(-4000, 4000, 1000), 
                      labels = c(seq(4000, 0, -1000), seq(1000, 4000, 1000))) + 
  scale_fill_brewer(palette = "Set1") 
ggsave(here("plots", "JAP_ITA_pyramid_time.pdf"))
```
...
d_long |> 
  ...
  ggplot(aes(year, pop)) + 
  geom_line() + 
  ggtitle("Japan population over time")
```
...
d_long |> 
  ...
  ggplot(aes(year, pop)) + 
  geom_line() + 
  ggtitle("Italy population over time")
```
...
d_long |> 
  ...
  ggplot(aes(year, pop)) + 
  geom_line() + 
  ggtitle("USA population over time")
```
```{r}
# get total populations across all age and sex
total_pops <- d  |>  
